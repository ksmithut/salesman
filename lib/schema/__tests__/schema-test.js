...
describe('lib/schema', () => {
  ...
  describe('constructor', () => {
    ...
    it('should be able to get the table name', () => {
      let schema = new Schema('Lead');

      expect(schema.tableName).to.be.equal('Lead');
      expect(() => schema.tableName = 'Foo').to.throw('Cannot set property tableName of #<Schema> which has only a getter');
    });

    it('should be able to get exports', () => {
      let schema = new Schema('Lead').export;

      expect(schema).to.have.property('tableName', 'Lead');
      expect(schema).to.have.property('attributes');
      expect(schema).to.have.property('methods');
      expect(schema).to.have.property('statics');
      expect(schema).to.have.property('hooks');
    });
  ...
  describe('.normalizeField', () => {
    ...
    it('should default to ref as the ref key', () => {
      expect(Schema.normalizeField('foo', { ref: 'Lead' })).to.be.eql({
        ref: 'Lead',
      });
    });

    it('should accept a different column key', () => {
      expect(Schema.normalizeField('foo', { $column: 'foobar' }, { columnKey: '$column' })).to.be.eql({
        column: 'foobar',
      });
    });

    it('should accept a different column key', () => {
      expect(Schema.normalizeField('foo', { $ref: 'Lead' }, { refKey: '$ref' })).to.be.eql({
        ref: 'Lead',
      });
    });
