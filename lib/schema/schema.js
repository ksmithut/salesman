...
class Schema {
   ...
   * @param {string} tableName - The name of the salesforce object
   ...
   */
  constructor(tableName, attributes, config) {
   ...
   * @description Exposes a way for you to inject common model functionality
   * while sharing one code base.
   * @param {function} plugin - A plugin function that accepts 2 arguments:
   * the schema instance, and the configuration respectively.
   * @param {object} [config] - The configuration for the plugin.
   * @returns {Schema} the schema instance
   */
  plugin(plugin, config) {
    plugin(this, Object.assign({}, config));
  ...
  /**
   * @name Schema#export
   * @description a convience property to export the properties needed for the
   * model
   */
  get export() {
    return {
  ...
  static normalizeAttributes(attributes, config, prefix) {
    ...
    Object.keys(attributes).forEach((key) => {
      ...
      let field = Schema.normalizeField(key, val, config);
   ...
   * column exists
   * @param {string} [refKey='ref'] - the ref key to use ot see if a
   * relationship exists
   ...
   */
  static normalizeField(key, val, config) {
    config = merge(true, DEFAULT_CONFIG, config);

    if (val === true) {
      val = { [config.columnKey]: key };
    } else if (typeof val === 'string') {
      val = { [config.columnKey]: val };
    }

    assert(val && typeof val === 'object' && !Array.isArray(val), `salesman: '${key}' has an invalid definition`);

    if (!val[config.columnKey] && !val[config.refKey]) { return false; }

    if (config.columnKey !== COLUMN_KEY) {
      val[COLUMN_KEY] = val[config.columnKey];
      delete val[config.columnKey];
    }

    if (config.refKey !== REF_KEY) {
      val[REF_KEY] = val[config.refKey];
      delete val[config.refKey];
    }
