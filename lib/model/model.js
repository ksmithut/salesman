...
class Model {
  ...
  find(queryObject) {
    return Bluebird
      .all([
        this.describe().then(normalizeQuery.bind(null, queryObject)),
        this.sobject(),
      ])
      .spread((query, sobject) => {
        return this.callHook('find', query, (modifiedQuery) => {
          let runnableQuery = sobject
            .select(modifiedQuery.select)
            .where(modifiedQuery.where)
            .limit(modifiedQuery.limit)
            .sort(modifiedQuery.sort)
            .skip(modifiedQuery.skip);

          return Bluebird.resolve(runnableQuery).map((row) => {
            row = this.format(row);
            this.schema.extendInstance(row);
            return row;
          });
        }, this);
      });
