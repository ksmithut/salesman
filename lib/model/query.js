...
class Query {
  ...
  select(val) {
    ...
    if (this[_query].select === '*') this[_query].select = null;
  ...
  subQuery(sobject, describe) {
    ...
    return Bluebird.reduce(Object.keys(query.include), (prev, field) => {
      ...
      let subModelName = describe.relationships[field].ref;
      let subModel = model.getModel(subModelName);
      let subModelObjectName = subModel.objectName;
      let childRelationship = describe.rawRelationships[subModelObjectName];

      assert(childRelationship && !childRelationship.deprecated, `${subModelName} is not a valid relationship on ${model.objectName} model`);

      prev.sobject = prev.sobject.include(childRelationship.relationshipName);
  ...
  exec() {
    ...
    return model.describeSobject()
      ...
      .map((row) => {
        ...
        console.log(row.Credit_Reports__r.records);
